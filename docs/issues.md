...
**발견일**: 2026-03-22
**심각도**: 중간
**상태**: ✅ 수정 완료 (2026-03-22) — 마커 있으면 교체, 마커 없고 heading 있으면 보호, 둘 다 없으면 신규 삽입
**상태**: ⚠️ v1.1.6에서 개선되었으나 잔존 이슈 있음

**수정 이력**:
- v1.1.5: `<!-- ai-setting:archetype-rules -->` 마커 기반 교체 도입 → 재적용 시 중복 방지 ✅
- v1.1.5 잔존: 기존 한국어 "## API 규칙" 유지 + 영문 "## API Rules" 추가 삽입 → 한/영 중복 ✗
- v1.1.6: 기존 비마커 "## API 규칙" 섹션을 **마커 블록으로 교체** → 중복 제거됨 ✅

**v1.1.6 잔존 문제**:
- 교체 시 영문("## API Rules")으로 삽입됨 → 한국어 프로젝트에서는 내용이 영문으로 바뀜
- 사용자가 한국어로 작성한 규칙이 영문으로 덮어써짐
- 다국어 프로젝트에서 archetype 규칙의 언어를 프로젝트 언어에 맞추는 옵션이 없음

**추가 수정 제안**:
- `--lang ko` 같은 옵션으로 archetype 규칙 삽입 언어를 지정할 수 있게
- 또는 마커 블록 교체 시 기존 내용의 언어를 감지하여 동일 언어로 유지
