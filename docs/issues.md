...
메모:
- 이 문서는 각 이슈가 발견된 당시의 증상, 가설, 수정 제안, 검증 메모를 보존하는 이력 문서입니다.
- 본문에 남아 있는 "현재", "수정 제안", "사용 불가" 같은 표현은 해당 이슈를 기록하던 시점 기준일 수 있습니다.
- 최신 동작과 우선순위는 `README*`, `docs/reference*`, `docs/roadmap.md`, `docs/plans/*`를 우선 기준으로 봅니다.
