...
> "문서만 있으면 문서를 따르고, 구현이 있으면 실제 상태를 먼저 본다"
...
`ai-setting`은 프로젝트를 아래 3가지 모드 중 하나로 분류해서 처리한다.
...
현재 상태:
- `init.sh`에서 기본 휴리스틱으로 `docs-first / hybrid / code-first` 자동 감지를 수행
...
다음 신호를 조합해 `docs-first / hybrid / code-first`를 정한다.
