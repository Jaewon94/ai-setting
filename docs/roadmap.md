...
> "필요한 도구 설정만 생성하고, 안 쓰는 도구는 건너뛴다"

현재는 init 실행 시 5개 도구 설정이 전부 생성된다. 사용자 피드백이 생기면 제외 옵션을 추가한다.

```bash
# 제외 방식 (기본은 전부 설치, 빼고 싶은 것만 지정)
./init.sh --no-cursor --no-gemini /path/to/project

# 또는 포함 방식
./init.sh --tools claude,cursor /path/to/project
```

현재 판단: 있어도 해가 없으므로 실제 사용자 요청이 있을 때 구현.
